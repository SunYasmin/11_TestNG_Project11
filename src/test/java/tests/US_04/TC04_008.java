...
public class TC04_008 {
    ...
    //13-"Ürün" "Fiyat" "Miktar"  "Ara Toplam"  yazisinin görüldüğünü test edilir
    //14-(+)  ve  (-)  butonlarinin arttirma ve eksiltme yaptigi test edilir
    //15- Update cart butonunun gorunur oldugu test edilir ve tiklanir
    //16-Stok fazlasi urun eklenemeyecegi test edilir
    //17_Shipping ve adres bilgilerinin gorunurlugu test edilir
    //18-Coupon seceneginin gorunur oldugu test edilir
    //19-Continue Shopping butonunun gorunur oldugu test edilir ve tiklanir
    //20-Proceed To Checkout butonunun gorunur oldugu test edilir ve tiklanir
    //21- Acilan sayfada Billing Details in gorunurlugu test edilir
    ...
    public void testCase04() throws InterruptedException {
        ...
        //TC08 Proceed to Checkout butonu olmali ve tiklaninca billing details gelmeli
        ReusableMethods.scrollIntoView(cartPage.applyCoupon);
        ReusableMethods.waitFor(3);
        ReusableMethods.waitFor(3);
        actions.moveToElement(cartPage.proceedToCheckoutButton);
        ReusableMethods.waitFor(3);
        actions.moveToElement(cartPage.proceedToCheckoutButton).click();
        ReusableMethods.waitFor(3);
        Assert.assertTrue(cartPage.proceedToCheckoutButton.isDisplayed());
        ReusableMethods.waitFor(3);
        Driver.getDriver().get("(//*[@href='https://pearlymarket.com/checkout-2/'])[3]");
        //actions.moveToElement(cartPage.proceedToCheckoutButton).click();
        ReusableMethods.waitFor(4);
